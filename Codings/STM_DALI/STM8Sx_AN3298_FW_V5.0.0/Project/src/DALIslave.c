...
#ifdef USE_RANDOM_RESPONSE_TIME
u8 RandomResponse = 0; // value from 0 - 32 defines response delay in range 7 - 16 Te
#endif //USE_RANDOM_RESPONSE_TIME
...
void receive_data() {
  ...
  #ifdef USE_RANDOM_RESPONSE_TIME
  // generate random answer time in range 0 - 32
  RandomResponse = get_timer_count();
  #endif //USE_RANDOM_RESPONSE_TIME
}
...
void send_data(u8 byteToSend)
{
  ...
  if(tick_count > 31)  // check for minimum response time
  {
  #ifdef USE_RANDOM_RESPONSE_TIME
    if(RandomResponse > (tick_count - 31))
      RandomResponse -= (tick_count - 31);
    else
      RandomResponse = 0;
  #endif //USE_RANDOM_RESPONSE_TIME
    tick_count = 31;
...
{
  #ifdef USE_RANDOM_RESPONSE_TIME
  //wait for added random response time ticks (RandomResponse = 0 - 32  => random answer in 7 - 16 Te)
  if(RandomResponse)
  {
    RandomResponse--;
    return;
...
// Counting time from last received command to calculate answer time
void count_answer_tick(void)
{
  tick_count++;
}
